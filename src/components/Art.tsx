...
export const Art: React.FC<any> = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => setSeconds(previousValue => previousValue + 1), 100);
  ...
  const [cells, setCells] = useState([[<div key={ 'test' }/>]]);
  useEffect(() => {
    ...
    for (let y = 0; y < rows; y++) {
      ...
      for (let x = 0; x < cols; x++) {
        ...
        blob[y][x] = chance > 6 ? <Cell position={ [x, y] } key={ `${ x },${ y }` }/> : <div key={ `${ x },${ y }` }/>;
      }
    }
    setCells(blob);
  }, [size, seconds]);
